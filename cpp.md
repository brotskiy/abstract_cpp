...
**using-объявление для функции** имеет следующую форму:

```Cpp
using MyNamespace::myFunc;
```

Оно вводит имя функции в текущую область видимости (блок кода, класс или пространство имен) и делает его доступным для использования без указания квалификатора `::`. Зачастую using-объявление применяется для того, чтобы:

+ позволить данной функции участвовать в 

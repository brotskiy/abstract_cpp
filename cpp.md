...
Если в результате поиска будет найдено несколько перегрузок, некоторый формальный аргумент которых соответствует *НА ОДНОМ УРОВНЕ* фактическому аргументу вызова, то этот вызов считается **неоднозначным** - происходит *ошибка компиляции*.

Если перегруженная функция принимает *несколько аргументов*, то наиболее подходящей перегрузкой будет являться та, все формальные аргументы которой соответствуют фактическим аргументам вызова, и при этом *минимальный уровень соответствия* имеет наибольшее значение среди остальных перегрузок.
